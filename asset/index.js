define([],function(){
  ...
  return {
    ...
    initScroll:function($win,$index){
    	var count=$win.attr('data-count')-$win.attr('data-extcount');
    	var lastScrollLeft=0;
    	var checkScroll=function(){
    		var win=$win[0];
    		var sw=win.scrollWidth,
    		cw=win.clientWidth *.95,
    		sl=win.scrollLeft,
    		index=0;
        ...
        var scrollto=Math.round(index * cw)
        index=index%count;
        //console.log(index)
    		//Scroll.maxScroll=cw * (count);

    		Scroll.to(win,scrollto,function(){
    			//console.log('done')
          if(!index){
            win.scrollLeft=0;
          }
